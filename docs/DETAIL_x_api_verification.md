...
API DM送信は基本導線とする。ただし、以下をすべて満たすことを前提に運用する。

- `dm.write` スコープが利用できる
- 実テストで1件のDM送信に成功する
- 送信成功レスポンスから証跡に必要なIDを保存できる
- 送信不可・権限不足・レート制限のエラーを分類できる
- クライアントがX API利用リスクと手動送信フォールバックの使い分けを理解している
...
| DM送信不可 | API送信を無効化し、手動送信フォールバックに切り替える |
...
- 手動送信フォールバック: 受信側条件、レート制限、クレジット制約の回避策として継続
